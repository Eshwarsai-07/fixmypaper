  ...
  return (
    <DashboardShell caption="Student Dashboard">
      ...
      <section className="bg-white border border-line rounded-xl p-6 shadow-sm">
        <h2 className="text-xl font-serif font-bold mb-2">Submission Format</h2>
        <p className="text-[13px] text-ink-soft mb-4 font-medium">Select the institutional format your paper should match.</p>
        <select
          value={formatId}
      ...
      {phase === "upload" && (
        <section className="bg-white border border-line rounded-card p-5 animate-fade-in">
          ...
          >
            ...
            <h3 className="font-serif font-bold text-lg mb-2">Upload Manuscript</h3>
            <p className="text-sm text-ink-soft mb-6 font-medium">
              Drop your PDF here, or click to browse. Max size 50 MB.
            </p>
            <input
              id="file-input"
              type="file"
            ...
            <button
              type="button"
              className="px-6 py-3 rounded-lg bg-brand text-white text-sm font-bold hover:bg-brand-dark transition-all shadow-md"
...
function ResultsView({ result, activeTab, setActiveTab, filter, setFilter, onReset }) {
  ...
  return (
    <section className="bg-white border border-line rounded-card p-5 animate-fade-in">
      {/* Header */}
      <div className="flex flex-wrap items-center justify-between gap-4 mb-6 border-b border-line pb-6">
        <h2 className="text-2xl font-serif font-bold text-brand">Quality Report Ready</h2>
        <a
...
function KPI({ value, label }) {
  return (
    <div className="bg-white border border-line rounded-xl px-4 py-5 text-center shadow-sm">
      <p className="text-2xl font-serif font-bold text-brand">{value}</p>
      <p className="text-[10px] text-ink-soft mt-1 font-bold uppercase tracking-widest">{label}</p>
    </div>
  );
}
