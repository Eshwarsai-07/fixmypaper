...
class PDFErrorDetector:
    ...
    def _extract_with_grobid(self, pdf_path: str):
        ...
        """
        try:
            print(f"[GROBID] Processing PDF with GROBID for job {self.job_id}...")
            self.module_status["grobid"] = "processing"
            
            @retry(stop=stop_after_attempt(3) | stop_after_delay(20), wait=wait_exponential(multiplier=1, min=2, max=10))
            def call_grobid():
            ...
            response = call_grobid()

            if response.status_code != 200:
                print(f"[GROBID] Error: status code {response.status_code}")
                self.module_status["grobid"] = "failed"
                self._tei_root = None
                return

            self.module_status["grobid"] = "success"
    ...
    def detect_errors(
        ...
        def _run_current_layer() -> None:
            try:
                ...
                self.module_status["tables"] = "processing"
                @retry(stop=stop_after_attempt(2) | stop_after_delay(30), wait=wait_exponential(multiplier=1, min=4, max=10))
                def run_tables():
                    self._extract_tables(pdf_path)
                run_tables()
                self.module_status["tables"] = "success"
        ...
        def _run_pix2text_layer() -> None:
            self.module_status["equations"] = "processing"
            try:
                @retry(stop=stop_after_attempt(2) | stop_after_delay(40), wait=wait_exponential(multiplier=1, min=5, max=15))
                def run_p2t():
                    return extract_equations_from_pdf(pdf_path)
                ...
                self.module_status["equations"] = "success"
            except Exception as exc:
                ...
                self.module_status["equations"] = "failed"
...
def process_pdf(
    ...
    Args:
        ...
        job_id              – unique identifier for logging and status tracking
    ...
    errors, doc, statistics = detector.detect_errors(input_path, required_sections, job_id=job_id)
    ...
    detector.module_status["annotation"] = "processing"
    try:
        detector.annotate_pdf(doc, errors, output_path)
        detector.module_status["annotation"] = "success"
    except Exception as e:
        print(f"[ANNOTATION] Error: {e}")
        detector.module_status["annotation"] = "failed"
        # Fallback: save without annotations or handle error
        doc.save(output_path, garbage=4, deflate=True)
        doc.close()

    extracted_data = detector.export_extracted_data()
    # Include module_status in extracted_data
    extracted_data["module_status"] = detector.module_status
